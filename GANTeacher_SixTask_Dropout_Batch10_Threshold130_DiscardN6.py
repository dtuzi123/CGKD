...
class LifeLone_MNIST(object):
    ...
    def Calculate_FID_ByAllLeanredComponents(self):
        ...
        for kk in range(np.shape(self.GeneratorArr)[0] - 1):
            arr1 = []
    ...
    def Discard(self):
        ...
        #self.discard_disirNum = np.shape(self.GeneratorArr)[0] - self.discardN
        for t1 in range(ComponentCount):
            if np.shape(GeneratorArr)[0] <= self.discardN:
                break
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
                ...
                if t1 > totalCount2:
                    if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                        ...
                        if fidScore > threshold:
                            trainingStep = 0
                            print("Expansion")
                            if np.shape(self.GeneratorArr)[0] > self.discardN:
                                self.discard_generator = []
                                self.Discard()
            ...
            for i in range(np.shape(self.GeneratorArr)[0]):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Oral_Threshold160_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
            ...
            for i in range(np.shape(self.GeneratorArr)[0]):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Reduced_Threshold160_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
            ...
            self.SaveImage("Threshold160_SixTask_realbatch", realbatch)
            self.SaveImage("Threshold160_SixTask_reco", myReco)

            self.SaveImage("Threshold160_SixTask_VAE1", reco1)
            self.SaveImage("Threshold160_SixTask_VAE2", reco2)
            self.SaveImage("Threshold160_SixTask_VAE3", reco3)
            self.SaveImage("Threshold160_SixTask_VAE4", reco4)
            self.SaveImage("Threshold160_SixTask_VAE5", reco5)
            self.SaveImage("Threshold160_SixTask_VAE6", reco6)
