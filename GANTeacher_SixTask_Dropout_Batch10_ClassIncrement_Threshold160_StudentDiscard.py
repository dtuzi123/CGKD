...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.discard_generator = []

        self.splitedMnist = self.Give_SplitedDataset(self.mnist_train_x, self.mnist_label)
        ...
        self.splitedSVHN = self.Give_SplitedDataset(self.svhn_train_x, self.svhn_label)

        self.FashionTrain_x, self.FashionTrain_label, self.FashionTest_x, self.FashionTest_label = GiveFashion32()

        self.splitedFashion = self.Give_SplitedDataset(self.FashionTrain_x, self.FashionTrain_label)
        ...
        self.splitedInverseFashion = self.Give_SplitedDataset(self.InverseFashionTrain_x,
                                                              self.InverseFashionTrain_label)
        ...
        self.splitedCIFAR = self.Give_SplitedDataset(self.cifar_train_x, self.cifar_train_label)
    ...
    def GiveInteplationResults(self,x1,x2):
        z1 = self.sess.run(self.Student_z,feed_dict={self.inputs:x1})
        z2 = self.sess.run(self.Student_z,feed_dict={self.inputs:x2})
    ...
    def InterplationResults(self):
        mnistX1 = self.SelectedSamples_byClasses(self.mnist_test_x,self.mnist_label_test,0)
        RmnistX1 = self.SelectedSamples_byClasses(self.InverseMNISTTest_x,self.InverseMNISTTest_label,0)

        mnistX2 = self.SelectedSamples_byClasses(self.mnist_test_x,self.mnist_label_test,1)
        RmnistX2 = self.SelectedSamples_byClasses(self.InverseMNISTTest_x,self.InverseMNISTTest_label,1)


        mnistX3 = self.SelectedSamples_byClasses(self.mnist_test_x,self.mnist_label_test,3)
        RmnistX3 = self.SelectedSamples_byClasses(self.InverseMNISTTest_x,self.InverseMNISTTest_label,3)

        mnistX5 = self.SelectedSamples_byClasses(self.mnist_test_x,self.mnist_label_test,5)
        RmnistX5 = self.SelectedSamples_byClasses(self.InverseMNISTTest_x,self.InverseMNISTTest_label,5)


        FashionX1 = self.SelectedSamples_byClasses(self.FashionTest_x,self.FashionTest_label,0)
        IFashionX1 = self.SelectedSamples_byClasses(self.InverseFashionTest_x,self.InverseFashionTest_label,0)

        FashionX3 = self.SelectedSamples_byClasses(self.FashionTest_x,self.FashionTest_label,3)
        IFashionX3 = self.SelectedSamples_byClasses(self.InverseFashionTest_x,self.InverseFashionTest_label,3)

        FashionX5 = self.SelectedSamples_byClasses(self.FashionTest_x,self.FashionTest_label,5)
        IFashionX5 = self.SelectedSamples_byClasses(self.InverseFashionTest_x,self.InverseFashionTest_label,5)


        recoX1 = self.GiveInteplationResults(mnistX1,RmnistX1)
        recoX2 = self.GiveInteplationResults(mnistX3,RmnistX3)
        recoX3 = self.GiveInteplationResults(mnistX5,RmnistX5)
        recoX5 = self.GiveInteplationResults(mnistX2,RmnistX2)

        recoX1_ = self.GiveInteplationResults(FashionX1,IFashionX1)
        recoX2_ = self.GiveInteplationResults(FashionX3,IFashionX3)
        recoX3_ = self.GiveInteplationResults(FashionX5,IFashionX5)

        self.SaveImage2("Reservoir_mnist_inter_1",recoX1)
        self.SaveImage2("Reservoir_mnist_inter_2",recoX2)
        self.SaveImage2("Reservoir_mnist_inter_3",recoX3)
        self.SaveImage2("Reservoir_mnist_inter_5",recoX5)

        self.SaveImage2("Reservoir_fashion_inter_1",recoX1_)
        self.SaveImage2("Reservoir_fashion_inter_2",recoX2_)
        self.SaveImage2("Reservoir_fashion_inter_3",recoX3_)

    def SelectedSamples_byClasses(self,data,labels,classLabel):
        labelsArr = np.argmax(labels, 1)
        batch = []
        for i in range(np.shape(data)[0]):
            if np.shape(batch)[0] == self.batch_size:
                break

            if labelsArr[i] == classLabel:
                batch.append(data[i])
    ...
    def Create_StuClassifier(self, index, selectedComponentIndex):
        ...
        continous_variables1_sub = z_mean_sub + z_log_sigma_sub * tf.random_normal(tf.shape(z_mean_sub), 0, 1,
                                                                                   dtype=tf.float32)
        self.Student_z = continous_variables1_sub
        ...
        continous_variables1_super = z_mean_super + z_log_sigma_super * tf.random_normal(tf.shape(z_mean_super), 0,
                                                                                         1, dtype=tf.float32)
        self.Student_Z = continous_variables1_super
    ...
    def build_model(self):
        ...
        self.z1 = tf.placeholder(tf.float32, [self.batch_size, self.z_dim])
        ...
        self.reco_z_loss = tf.reduce_mean(tf.reduce_sum(tf.square(self.z1 - self.z2), [1]))
    ...
    def MakePseudoData(self):
        ...
        for i in range(np.shape(self.GeneratorArr)[0]-1):
            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
    ...
    def Student_Evaluation(self, set1, set2):
        count = int(np.shape(set1)[0] / self.batch_size)
        sumLoss = 0
        for i in range(count):
            batch1 = set1[i * self.batch_size:(i + 1) * self.batch_size]
            batch2 = set2[i * self.batch_size:(i + 1) * self.batch_size]
            z1 = self.sess.run(self.Student_z, feed_dict={self.inputs: batch1})
            z2 = self.sess.run(self.Student_z, feed_dict={self.inputs: batch2})
            loss1 = self.sess.run(self.reco_z_loss, feed_dict={self.z1: z1, self.z2: z2})
            sumLoss = sumLoss + loss1
        sumLoss = sumLoss / count
        return sumLoss

    def Generate_GraphRelationship(self, generatorArr, componentArr):
        ComponentCount = np.shape(generatorArr)[0]
        totalSet = []

        dataCount = 20
        for kk in range(ComponentCount):
            arr1 = []
            for k1 in range(dataCount):
                batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)

                ComponentIndex = kk
                generatedImages = self.sess.run(generatorArr[ComponentIndex], feed_dict={self.z: batch_z})
                generatedImages = np.reshape(generatedImages, (self.batch_size, 32, 32, 3))
                # generatedImagesB = np.concatenate((generatedImages, generatedImages), axis=3)
                # generatedImagesB = np.concatenate((generatedImagesB, generatedImages), axis=3)
                generatedImagesB = generatedImages
                if np.shape(arr1)[0] == 0:
                    arr1 = generatedImagesB
                else:
        ...
        for i in range(ComponentCount):
            ComponentIndex_i = i  # componentArr[i]

            set1 = totalSet[i]
            for j in range(i + 1, ComponentCount):
                ComponentIndex_j = j  # componentArr[j]

                set2 = totalSet[j]
                nodeArr2.append(ComponentIndex_j)
                nodeArr1.append(ComponentIndex_i)

                mse1 = self.Student_Evaluation(set1, set2)
                mse1 = float(mse1)
                graph.append(mse1)

        # arr1 = np.array(arr1)

        return graph, nodeArr1, nodeArr2
    ...
    def Make_Choice(self, index1, index2, componentArr, GeneratorArr):
        componentCount = np.shape(GeneratorArr)[0]
        ...
        for j in range(dataCount):
            ...
            #generatedImagesB = np.concatenate((generatedImagesB, generatedImages), axis=3)
            generatedImagesB = generatedImages
            if np.shape(set1)[0] == 0:
                set1 = generatedImagesB
            else:
                set1 = np.concatenate((set1, generatedImagesB), axis=0)
        ...
        for j in range(dataCount):
            ...
            #generatedImagesB = np.concatenate((generatedImagesB, generatedImages), axis=3)
            generatedImagesB = generatedImages
            if np.shape(set2)[0] == 0:
                set2 = generatedImagesB
            else:
                set2 = np.concatenate((set2, generatedImagesB), axis=0)
        ...
        for kk in range(componentCount):
            componentindex3 = kk
            if componentindex3 == index1:
                continue
            if componentindex3 == index2:
                continue
            ...
            otherSet.append(arr1)

        sum1 = 0
        sum2 = 0
        for i in range(np.shape(otherSet)[0]):
            tSet = otherSet[i]
            mse1 = self.Student_Evaluation(set1, tSet)
            sum1 = sum1 + mse1

            mse2 = self.Student_Evaluation(set2, tSet)
            sum2 = sum2 + mse2
    ...
    def Discard(self):
        ...
        ComponentCount = np.shape(self.GeneratorArr)[0]  # self.componentCount
        componentArr = []
        lossArr = []
        GeneratorArr = []
        TeacherPredictions = []
        for i in range(ComponentCount):
            componentArr.append(i)
            # lossArr.append(self.lossArr[i])
            GeneratorArr.append(self.GeneratorArr[i])
            # TeacherPredictions.append(self.classifierModelArr[i])

        self.discard_disirNum = np.shape(self.GeneratorArr)[0] - 6
        for t1 in range(self.discard_disirNum):
            arr1, nodeArr1, nodeArr2 = self.Generate_GraphRelationship(GeneratorArr, componentArr)
            componentIndex = np.argsort(arr1)

            index1 = nodeArr1[componentIndex[0]]
            index2 = nodeArr2[componentIndex[0]]

            deltedComponentIndex = self.Make_Choice(index1, index2, componentArr, GeneratorArr)

            deletedIndex = deltedComponentIndex

            if deletedIndex >= 0:
                # del lossArr[deletedIndex]
                del GeneratorArr[deletedIndex]
                del componentArr[deletedIndex]
                # del TeacherPredictions[deletedIndex]

        for j in range(np.shape(GeneratorArr)[0]):
            # self.discard_vaeloss.append(lossArr[j])
            self.discard_generator.append(GeneratorArr[j])
    ...
    def Give_SplitedDataset(self,trainingX,trainingY):

        arr1, labelArr1, arr2, labelArr2, arr3, labelArr3, arr4, labelArr4, arr5, labelArr5 = Split_dataset_by5(
            trainingX,
            trainingY)

        rTrainingX = np.concatenate((arr1,arr2,arr3,arr4,arr5),axis=0)
        return rTrainingX
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.splitedInverseMNIST = self.Give_SplitedDataset(self.InverseMNISTTrain_x, self.InverseMNISTTrain_label)
            ...
            self.totalSet = np.concatenate((self.splitedMnist, self.splitedSVHN, self.splitedFashion,
                                            self.splitedInverseFashion, self.splitedInverseMNIST, self.splitedCIFAR),
                                           axis=0)

            self.totalLabel = np.concatenate((self.mnist_label, self.svhn_label, self.FashionTrain_label,
                                              self.InverseFashionTrain_label, self.InverseMNISTTrain_label,
                                              self.cifar_train_label), axis=0)
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if np.shape(self.TeacherClassifier)[0] == 1:
                    ...
                    memorySet = self.FixedMemory
                    for epochs in range(self.epoch):
                        n_examples = np.shape(memorySet)[0]
                        index2 = [i for i in range(n_examples)]
                        np.random.shuffle(index2)
                        memorySet = memorySet[index2]

                        for t5 in range(count3):
                            batchImages = memorySet[t5 * self.batch_size:(t5 + 1) * self.batch_size]
                ...
                if t1 > totalCount2:
                    if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                        ...
                        if fidScore > threshold:
                            ...
                            if np.shape(self.GeneratorArr)[0] > 6:
                                self.discard_generator = []
                                self.Discard()
                                self.GeneratorArr = self.discard_generator
            ...
            is1 = self.Calculate_IS_Results(self.mnist_test_x)
            is2 = self.Calculate_IS_Results(self.svhn_test_x)
            is3 = self.Calculate_IS_Results(self.FashionTest_x)
            is4 = self.Calculate_IS_Results(self.InverseFashionTest_x)
            is5 = self.Calculate_IS_Results(self.InverseMNISTTest_x)
            is6 = self.Calculate_IS_Results(self.cifar_test_x)

            sum = is1 + is2 + is3 + is4 + is5 + is6
            sum = sum / 6.0
            ...
            self.SaveImage("Class_Threshold160_SixTask_realbatch", realbatch)
            self.SaveImage("Class_Threshold160_SixTask_reco", myReco)

            myarr = realbatch[0:2]
            myarr2 = myReco[0:2]
            myarr = np.concatenate((myarr, realbatch[11:13]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[11:13]), axis=0)

            myarr = np.concatenate((myarr, realbatch[22:24]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[22:24]), axis=0)

            myarr = np.concatenate((myarr, realbatch[32:34]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[32:34]), axis=0)

            myarr = np.concatenate((myarr, realbatch[42:44]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[42:44]), axis=0)

            myarr = np.concatenate((myarr, realbatch[56:58]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[56:58]), axis=0)

            self.SaveImage_Small("Class_Threshold160_SixTask_ReaLSmall", myarr)
            self.SaveImage_Small("Class_Threshold160_SixTask_RecoSmall", myarr2)

            self.SaveImage("Class_Threshold160_SixTask_VAE1", reco1)
            self.SaveImage("Class_Threshold160_SixTask_VAE2", reco2)
            self.SaveImage("Class_Threshold160_SixTask_VAE3", reco3)
            self.SaveImage("Class_Threshold160_SixTask_VAE4", reco4)
            self.SaveImage("Class_Threshold160_SixTask_VAE5", reco5)
            self.SaveImage("Class_Threshold160_SixTask_VAE6", reco6)
            ...
            for i in range(np.shape(self.GeneratorArr)[0]):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Class_Threshold160_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))

            self.InterplationResults()
            '''
