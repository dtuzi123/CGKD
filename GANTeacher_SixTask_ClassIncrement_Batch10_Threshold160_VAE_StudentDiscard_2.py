...
class LifeLone_MNIST(object):
    ...
    def Create_Teacher(self, index, selectedComponentIndex):
        ...
        generation = tf.reshape(reco2, (-1, 32, 32, 3))
        self.GeneratorArr.append(generation)
        ...
        self.VAERecoLoss = reconstruction_loss1
        ...
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            self.Teacher_optim1 = tf.train.AdamOptimizer(learning_rate=1e-4) \
        ...
        with tf.Session(config=config) as sess:
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if np.shape(self.TeacherClassifier)[0] == 1:
                    ...
                    for epoch in range(epochs):
                        ...
                        for idx in range(myCount):
                            ...
                            _ = self.sess.run(self.Teacher_optim1,
                ...
                print("epoch {0}/{1}, step {2}/{3}, train ELBO: {4:.2f}, val ELBO: {5:.2f}, time: {6:.2f}"
                      .format(t1, totalCount3, 0, 0, 0, 0, self.componentCount, ))
            ...
            self.SaveImage("Student_LIMix_SixTask_realbatch", realbatch)
            self.SaveImage("Student_LIMix_SixTask_reco", myReco)

            self.SaveImage("Student_LIMix_SixTask_VAE1", reco1)
            self.SaveImage("Student_LIMix_SixTask_VAE2", reco2)
            self.SaveImage("Student_LIMix_SixTask_VAE3", reco3)
            self.SaveImage("Student_LIMix_SixTask_VAE4", reco4)
            self.SaveImage("Student_LIMix_SixTask_VAE5", reco5)
            self.SaveImage("Student_LIMix_SixTask_VAE6", reco6)

            myarr = realbatch[0:4]
            myarr2 = myReco[0:4]
            myarr = np.concatenate((myarr, realbatch[11:15]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[11:15]), axis=0)

            myarr = np.concatenate((myarr, realbatch[22:26]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[22:26]), axis=0)

            myarr = np.concatenate((myarr, realbatch[32:36]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[32:36]), axis=0)

            myarr = np.concatenate((myarr, realbatch[42:46]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[42:46]), axis=0)

            myarr = np.concatenate((myarr, realbatch[56:60]), axis=0)
            myarr2 = np.concatenate((myarr2, myReco[56:60]), axis=0)

            self.SaveImage_Small("Student_LIMix_SixTask_ReaLSmall", myarr)
            self.SaveImage_Small("Student_LIMix_SixTask_RecoSmall", myarr2)
            ...
            for i in range(np.shape(self.GeneratorArr)[0]):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Student_LIMix_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
