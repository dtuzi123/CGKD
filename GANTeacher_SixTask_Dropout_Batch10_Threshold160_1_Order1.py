...
class LifeLone_MNIST(object):
    ...
    def Calculate_RecoAcc(self,trainX,trainY,testX,testY):
        count = int(np.shape(trainX)[0] / self.batch_size)
        trainReco = []
        for i in range(count):
            batch1 = trainX[i * self.batch_size : (i+1)*self.batch_size]
            reco = self.sess.run(self.StudentVAE,feed_dict={self.inputs:batch1,self.gan_inputs:batch1})
            if np.shape(trainReco)[0] == 0:
                trainReco = reco
            else:
                trainReco = np.concatenate((trainReco, reco),axis=0)
        trainRecoY = trainY[0:np.shape(trainReco)[0]]
        ...
        net.fit(trainReco, trainRecoY,
                                   verbose=0, epochs=10,
                                   shuffle=True)
        val = net.evaluate(testX,testY)[0]
        return val
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            acc1= self.Calculate_RecoAcc(self.mnist_train_x,self.mnist_label,self.mnist_test_x,self.mnist_label_test)
            acc2 = self.Calculate_RecoAcc(self.svhn_train_x,self.svhn_label,self.svhn_test_x,self.svhn_label_test)
            acc3 = self.Calculate_RecoAcc(self.FashionTrain_x,self.FashionTrain_label,self.FashionTest_x,self.FashionTest_label)
            acc4 = self.Calculate_RecoAcc(self.InverseFashionTrain_x,self.InverseFashionTrain_label,self.InverseFashionTest_x,self.InverseFashionTest_label)
            acc5 = self.Calculate_RecoAcc(self.InverseMNISTTrain_x,self.InverseMNISTTrain_label,self.InverseMNISTTest_x,self.InverseMNISTTest_label)
            acc6 = self.Calculate_RecoAcc(self.cifar_train_x,self.cifar_train_label,self.cifar_test_x,self.cifar_test_label)
            myacc = acc1 + acc2 + acc3 + acc4 + acc5 + acc6
            myacc = myacc / 6.0
            ...
            for i in range(self.componentCount):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Threshold160_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
