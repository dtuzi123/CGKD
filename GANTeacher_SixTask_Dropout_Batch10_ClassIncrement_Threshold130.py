...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.splitedInverseFashion = self.Give_SplitedDataset(self.InverseFashionTrain_x,self.InverseFashionTrain_label)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.totalSet = np.concatenate((self.splitedMnist,self.splitedSVHN,self.splitedFashion,self.splitedInverseFashion,self.splitedInverseMNIST,self.splitedCIFAR),axis=0)
            ...
            totalCount2 = int(totalCount2 / 10)
            ...
            self.SaveImage("SixTask_realbatch_CC_threshold" + str(self.threshold), realbatch)
            self.SaveImage("SixTask_reco_CC_threshold" + str(self.threshold), myReco)

            self.SaveImage("SixTask_VAE1_CC_threshold" + str(self.threshold), reco1)
            self.SaveImage("SixTask_VAE2_CC_threshold" + str(self.threshold), reco2)
            self.SaveImage("SixTask_VAE3_CC_threshold" + str(self.threshold), reco3)
            self.SaveImage("SixTask_VAE4_CC_threshold" + str(self.threshold), reco4)
            self.SaveImage("SixTask_VAE5_CC_threshold" + str(self.threshold), reco5)
            self.SaveImage("SixTask_VAE6_CC_threshold" + str(self.threshold), reco6)
            ...
            for i in range(self.componentCount):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Final_SixTask_CC_threshold" + str(self.threshold) + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
