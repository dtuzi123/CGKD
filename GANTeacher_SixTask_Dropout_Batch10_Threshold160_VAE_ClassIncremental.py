...
class LifeLone_MNIST(object):
    ...
    def Make_Choice(self, index1, index2, componentArr, GeneratorArr):
        ...
        for j in range(dataCount):
            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
            generatedImages1 = self.sess.run(GeneratorArr[index1], feed_dict={self.z: batch_z})
            generatedImages2 = self.sess.run(GeneratorArr[index2], feed_dict={self.z: batch_z})
            if np.shape(set1)[0] == 0:
                set1 = generatedImages1
            else:
                set1 = np.concatenate((set1,generatedImages1),axis=0)

            if np.shape(set2)[0] == 0:
                set2 = generatedImages2
            else:
                set2 = np.concatenate((set2,generatedImages2),axis=0)

        is1 = self.Calculate_IS(set1)
        is2 = self.Calculate_IS(set2)
        if is1 > is2:
            return index2
        else:
            return index1
