...
class LifeLone_MNIST(object):
    ...
    def InterplationResults(self):
        ...
        self.SaveImage2("mnist_inter_1",recoX1)
        self.SaveImage2("mnist_inter_2",recoX2)
        self.SaveImage2("mnist_inter_3",recoX3)

        self.SaveImage2("fashion_inter_1",recoX1_)
        self.SaveImage2("fashion_inter_2",recoX2_)
        self.SaveImage2("fashion_inter_3",recoX3_)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.totalSet = []
            self.totalLabel = []
            self.totalSet = np.concatenate((self.FashionTrain_x, self.svhn_train_x,
                                            self.InverseFashionTrain_x, self.InverseMNISTTrain_x, self.cifar_train_x,
                                            self.mnist_train_x),
                                           axis=0)

            self.totalLabel = np.concatenate((self.FashionTrain_label, self.svhn_label,
                                              self.InverseFashionTrain_label, self.InverseMNISTTrain_label,
                                              self.cifar_train_label, self.mnist_label), axis=0)
            ...
            for i in range(np.shape(self.GeneratorArr)[0]):
                ...
                newGan = newGan * 255.0
                newGan = np.reshape(newGan, (-1, 32, 32, 3))
                str2 = "Reduced_Threshold" +str(self.threshold) +"_Final_SixTask_" + str(i) + ".png"
                cv2.imwrite(os.path.join("results/", str2),
                            merge2(newGan[:64], [8, 8]))
            ...
            self.SaveImage("Threshold1" +str(self.threshold) +"_SixTask_realbatch", realbatch)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_reco", myReco)

            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE1", reco1)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE2", reco2)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE3", reco3)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE4", reco4)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE5", reco5)
            self.SaveImage("Threshold" +str(self.threshold) +"_SixTask_VAE6", reco6)
